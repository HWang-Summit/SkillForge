---
name: galaxypedia-karpathy-guidelines
description: 当维护 Galaxypedia 的 Markdown、脚本或 agent 指令时使用，遵循 Karpathy 风格的简单、直接、目标驱动改动。
---
...
可以参考当前用户提供的 Obsidian/Claude skills 源目录设计，但不要照搬；优先服务 Galaxypedia 的 `raw/`、`wiki/`、`SCHEMA.md`、`index.md` 和 `log.md` 约束。

- 当前应强化：`galaxypedia-wiki-ingest`、`galaxypedia-wiki-query`、`galaxypedia-wiki-lint`、`wiki` 作为路由入口。
