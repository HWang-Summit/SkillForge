...
| Zotero 文献、DOI、item key、collection、PDF 附件，或 `raw/papers` 中未完成摄入的 PDF | `skills/zotero-galaxypedia-bridge/SKILL.md`，再按 `galaxypedia-wiki-ingest` 摄入 bundle Markdown |
...
- 论文 PDF 的唯一事实源是 `raw/papers/pdf-<sha-prefix>/` bundle；新论文必须经过 `stage-* → 分类提案 → commit-bundle`。
...
- `zotero-galaxypedia-bridge`：为论文创建/复用 canonical bundle、MinerU 结果、分类提案和相对链接附件；成功后交给 wiki ingest。
- `galaxypedia-zotero-ingest`：仅用于历史 SQLite metadata/index 的只读核查，不参与新摄入。
...
- 先生成 raw source，再写 wiki：非论文 PDF、Office、图片先经 `galaxypedia-mineru-import`；网页文章 URL 先经 `galaxypedia-defuddle`；输出稳定 Markdown 后再进入 `galaxypedia-wiki-ingest`。
- Zotero 来源、外部论文 PDF 与 `raw/papers` 中未完成摄入的 PDF，先经 `zotero-galaxypedia-bridge` 的 `stage-* → 分类提案 → commit-bundle`；只在 `linked`/`ingested` 后摄入 `paper.mineru.md`。
