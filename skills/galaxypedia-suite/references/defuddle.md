...
迁移自 Galaxypedia `skills/defuddle/SKILL.md`。本文件是 `galaxypedia-suite` 的内部参考模块，不是 SkillForge v0.1.0 中独立安装的根目录技能。
...
- PDF、Office 文档、图片或明显指向文件下载的 URL 交给 `the installed MinerU skill`。
- 输出只能保存到 `raw/articles/`，作为后续 `wiki-ingest` 的 source。
...
如果 URL 已经被保存为 `raw/articles/*.md`，不要重复清洗，直接进入 `wiki-ingest`。
...
导入成功后继续使用生成的 `raw/articles/*.md` 作为本次 ingest source，并切回 `references/wiki-ingest.md`。
