...
迁移自 Galaxypedia `skills/json-canvas/SKILL.md`。本文件是 `galaxypedia-suite` 的内部参考模块，不是 SkillForge v0.1.0 中独立安装的根目录技能。
...
```json
{
  ...
  "width": 360,
  "height": 220,
  "file": "references/wiki-ingest.md",
  "color": "4"
}
```
...
- `wiki-ingest` 如何连接 `mineru-import`、`defuddle`、`zotero-ingest`。
- `wiki-query`、`wiki-lint`、`karpathy-guidelines` 的职责边界。
- `json-canvas` 作为可视化派生层能力。
