...
迁移自 Galaxypedia `skills/wiki-ingest/SKILL.md`。本文件是 `galaxypedia-suite` 的内部参考模块，不是 SkillForge v0.1.0 中独立安装的根目录技能。
...
1. 读 `SCHEMA.md`、`the `galaxypedia-suite` router`。
...
- `raw/**/*.md`：直接摄入。
- PDF、Office、图片、本地非 Markdown 文件：先读 `the installed MinerU skill`，用 MinerU 生成 `raw/.../*.mineru.md` 和同名资源目录，再摄入生成的 Markdown。
- 远程 URL：先判断类型。网页文章、博客、HTML 文档页读 `references/defuddle.md`，清洗并保存为 `raw/articles/*.md`；PDF、Office、图片 URL 读 `the installed MinerU skill`。
- Zotero 文献、DOI、item key、collection 或 Zotero PDF 附件：先读 `references/zotero-ingest.md`，从本地 Zotero 只读解析 metadata、collection 和可读 PDF 路径，再用 MinerU 生成 `raw/papers/<collection-path>/*.mineru.md`。
