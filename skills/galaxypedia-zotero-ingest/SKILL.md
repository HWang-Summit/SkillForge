...
description: 从本地 Zotero 文献库读取论文 metadata、collection 分类和 PDF 附件路径，并把可读 PDF 交给 MinerU 和 Galaxypedia wiki 摄入流程。
---
...
2. 如果要解析 PDF，继续读 `skills/galaxypedia-mineru-import/SKILL.md`。
3. 如果要把 MinerU Markdown 写入 wiki，继续读 `skills/galaxypedia-wiki-ingest/SKILL.md`。
...
```bash
python scripts/zotero_ingest.py --zotero-db "$ZOTERO_DB" --zotero-data-dir "$ZOTERO_DATA_DIR" list-collections
```
...
- `storage:<filename>.pdf` → `$ZOTERO_DATA_DIR/storage/<attachment-key>/<filename>.pdf`
...
5. MinerU 成功后，按 `galaxypedia-wiki-ingest` 摄入生成的 `.mineru.md`。
...
6. 每篇 MinerU 成功后，立即按 `galaxypedia-wiki-ingest` 摄入生成的 `.mineru.md`。
