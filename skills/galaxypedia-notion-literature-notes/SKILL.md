...
2. 根据来源类型读取相关 skill：
   - 未摄入的非论文 PDF/Office/图片：先读 `skills/galaxypedia-mineru-import/SKILL.md`，生成稳定 `raw/...*.mineru.md` 后再继续。
   - Zotero 来源、外部论文 PDF 或 `raw/papers` PDF：先读 `skills/zotero-galaxypedia-bridge/SKILL.md`；仅在 bundle 已 `linked`/`ingested` 后读取 manifest/source-index 中的 item、attachment、PDF 与 MinerU provenance。
...
- Galaxypedia MinerU Markdown：`raw/papers/pdf-<sha-prefix>/paper.mineru.md`
...
- 本机 `open ...pdf` 命令。
- 如果 manifest/source-index 有 Bridge provenance，记录 item key、attachment key、canonical PDF path、PDF hash 与分类 collection path。
