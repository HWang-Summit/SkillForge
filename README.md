...
本仓库当前维护 22 个独立 skill。`skills/_shared/` 是 `nature-*` 技能使用的共享支持目录，不作为独立 skill 触发。
...
| Skill | 用途 |
| --- | --- |
| `zotero-local-pdf-attach` | 将本地 PDF 分批、可恢复地附加为 Zotero Desktop 托管副本，并完成匹配、去重与审计。 |
| `zotero-cloud-library` | 管理 Zotero 云端文献库，支持添加或更新论文 metadata、摘要、标签和 collection。 |
